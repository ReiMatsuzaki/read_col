...
contains
  ...
  subroutine SymMat_new_read(this, ifile)
    type(SymMat) this
    integer ifile
    ...
    call SymMat_new(this, num)
    
  end subroutine SymMat_new_read
  subroutine SymMat_set_read(this, ifile)
    type(SymMat) this
    integer ifile
    integer num, iblk, ibuf
    integer int, idx, idx_block
    integer lbli(1080)
    integer mask1
    complex*16 spi(1080)
    ...
    do while(iblk .eq. 0)
       ...
       do int = 1, ibuf
          idx = idx + 1
          this % j(idx) = iand(lbli(int), mask1)
          this % i(idx) = iand(ishft(lbli(int), -15), mask1)
          this % isym(idx) = ishft(lbli(int), -26)
          this % val(idx) = spi(int)
       end do
    end do
    
  end subroutine SymMat_set_read
  subroutine SymMat_show(this)
    type(SymMat) this
    integer i
    write(*, *) this % num
    do i = 1, this % num
       write(*, *) this % i(i), this % j(i), this % isym(i), this % val(i)
    end do
...
contains
  ...
  subroutine AoInts_new_read(this)
    ...
    call SymMat_new_read(this % t_mat, ifile)
    call SymMat_new_read(this % v_mat, ifile)
    call SymERI_new_read(this % eri, ifile)

    rewind ifile

    call AoInts_read_header(this, ifile)
    call SymMat_set_read(this % s_mat, ifile)
  ...
  subroutine AoInts_delete(this)
    type(AoInts) this
    call SymMat_delete(this % s_mat)
    call SymMat_delete(this % t_mat)
    call SymMat_delete(this % v_mat)
