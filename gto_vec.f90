...
program main
  ...
  type(IntIn) int_in
  real*8 :: r0 = 10.0d0
  integer, parameter :: ofile = 13

  call IntIn_new_read(int_in, 5, .false.)
  
  open(unit = ofile, file = "delta.dat")
  
  call calc_delta_ylm(int_in, r0, 1, 0, ofile)
  close(unit = ofile)

contains
  subroutine calc_delta_ylm(int_in, r0, L, M, ofile)
