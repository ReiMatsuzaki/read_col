...
contains
  subroutine test_SymBlockMat()
    ...
    call Expect_Eq("1,1,1,1", (1.0d0, 0.0d0), val)
    call SymBlockMat_get(mat, 1, 1, 1, 2, val)
    call Expect_Eq("1,1,1,2", (1.0d0, 0.1d0), val)
    call SymBlockMat_get(mat, 1, 2, 1, 2, val)
    call Expect_Eq("1,2,1,2", (5.0d0, 0.1d0), val)
    ...
    call expect_false("Block size(1,3)", have_val)

    call SymBlockMat_get_block_size(mat, 1, 2, num_i, num_j ,have_val)
    call expect_true("block size(1,2)", have_val)
    call Expect_Eq("block size(1,2)", 2, num_i)
    call Expect_Eq("block size(1,2)", 1, num_j)
